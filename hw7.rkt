        ...
        [(ssendE obj-expr class-name method-name arg-expr)
         (local [(define obj (recur obj-expr))
           ...
           (call-method class-name method-name classes
                        obj arg-val))]
        [(selectE pos obj)
         (let [(position (recur pos)) (object (recur obj))]
           (cond [(not (numV? position)) (error 'interp "not a number")]
                 [(not (objV? object)) (error 'interp "not an object")]
                 [else (if (= (numV-n position) 0)
                         (call-method (objV-class-name object) 'zero classes object arg-val)
                         (call-method (objV-class-name object) 'nonzero classes object arg-val)
                         )]))]
        [(instanceofE obj super-name)
           (type-case Value (recur obj)
            [(objV obj-class _)
              (if (eq? obj-class super-name)
                (numV 0)
                (numV (instanceof obj-class super-name classes)))]
            [else (error 'interp "not an object")])]))))

(define (instanceof object-name super-name classes)
  (cond [(eq? object-name super-name) 1]
        [(eq? super-name 'Object) 0]
        [else (let [(super-name (classC-super-name (find classes super-name)))]
                (instanceof object-name super-name classes)
                )]))
    ...
    (type-case ExpI a
      ...
      [(superI method-name arg-expr)
       (ssendE (thisE)
               ...
               method-name
               (recur arg-expr))]
      [(instanceofI object super-name)
       (instanceofE (recur object) super-name)])))
...
(module+ test
  ...
  (test/exn (interp-prog (list `{class Fish extends Object
                           {size color}})
                     `{instanceof {new Object} NotAClass})
        "find: not found: NotAClass"))
