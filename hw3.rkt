...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(delayE body) (delayV body env)]
    [(forceE body) (let ([interped-body (interp body env)])
                     (if (delayV? interped-body)
                         (interp (delayV-body interped-body) (delayV-env interped-body))
...
;; unlet ----
(define (unlet-env [s : Symbol] [env : Env]) : Env
  (cond [(eq? s (bind-name (first env))) (rest env)]
        [else (cons (first env) (unlet-env s (rest env)))])
  )
