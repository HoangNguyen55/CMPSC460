...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{if0 ANY ANY ANY} s)
     (let ([exps (rest (s-exp->list s))])
       (if0 (parse (first exps)) (parse (second exps)) (parse (third exps))))]
    [(s-exp-match? `{pair ANY ANY} s)
     (let ([exps (rest (s-exp->list s))])
       (pair (parse (first exps)) (parse (second exps))))]
    [(s-exp-match? `{fst ANY} s)
     (let ([exps (second (s-exp->list s))])
       (fst (parse exps))) ]
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(if0 val true false) (if (= (numV-n (interp val env)) 0) (interp true env) (interp false env))]
    [(fst pair) (force (pairV-lhs (interp pair env)))]
    [(snd pair) (force (pairV-rhs (interp pair env)))]
    [(pair lhs rhs) (pairV (delay lhs env (box (none))) (delay rhs env (box (none))))]
